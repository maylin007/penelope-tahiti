...
export default function Home() {
  ...
  const collections = collectionImages.map((col, i) => ({
    ...col,
    title: t(T.collectionsData[i].title, lang),
    description: t(T.collectionsData[i].description, lang),
  }));
    ...
    <>
        ...
        <div className="relative z-10 text-center px-6 max-w-3xl mx-auto">
          ...
          <AnimatedSection delay={0.2}>
            <h1 className="font-[family-name:var(--font-display)] text-5xl sm:text-6xl md:text-7xl font-light text-white leading-[1.1] mb-8">
              {t(T.heroTitle1, lang)}
              <br />
              <span className="text-brand-light">{t(T.heroTitle2, lang)}</span>
            </h1>
          </AnimatedSection>
        ...
        <div className="max-w-7xl mx-auto">
          <AnimatedSection>
            <div className="text-center mb-20">
              <p className="text-[11px] tracking-[0.3em] uppercase text-accent mb-4">{t(T.collectionsLabel, lang)}</p>
              <h2 className="font-[family-name:var(--font-display)] text-4xl sm:text-5xl font-light text-dark">{t(T.collectionsTitle, lang)}</h2>
            </div>
          </AnimatedSection>
      ...
      <section className="py-28 px-6 bg-light">
        <div className="max-w-5xl mx-auto text-center">
          <AnimatedSection>
            ...
            <h2 className="font-[family-name:var(--font-display)] text-4xl font-light text-dark mb-16">{t(T.brandsTitle, lang)}</h2>
          </AnimatedSection>
          <AnimatedSection delay={0.2}>
      ...
      <section>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2">
          ...
          <AnimatedSection className="flex items-center px-8 lg:px-20 py-20">
            <div>
              <p className="text-[11px] tracking-[0.3em] uppercase text-accent mb-4">{t(T.philosophyLabel, lang)}</p>
              <h2 className="font-[family-name:var(--font-display)] text-3xl sm:text-4xl font-light text-dark mb-8 leading-tight">{t(T.philosophyTitle, lang)}<br />{t(T.philosophyTitle2, lang)}</h2>
              <p className="text-muted leading-relaxed mb-6 text-[15px]">{t(T.philosophyP1, lang)}</p>
              <p className="text-muted leading-relaxed mb-10 text-[15px]">{t(T.philosophyP2, lang)}</p>
              <Link href="/collections" className="inline-block border border-dark text-dark px-8 py-4 text-[13px] tracking-wider uppercase transition-all duration-300 hover:bg-dark hover:text-white">{t(T.philosophyBtn, lang)}</Link>
            </div>
          </AnimatedSection>
        </div>
      </section>
      ...
      <section className="py-28 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <AnimatedSection>
            <p className="font-[family-name:var(--font-display)] text-2xl sm:text-3xl font-light text-muted italic leading-relaxed mb-6">&ldquo;{t(T.quote, lang)}&rdquo;</p>
            <div className="w-8 h-px bg-brand mx-auto mb-4" />
            <p className="text-[13px] tracking-wide uppercase text-muted">{t(T.quoteName, lang)}</p>
          </AnimatedSection>
        </div>
      </section>
    </>
