...
export default function CollectionsContent() {
  ...
  const T = translations.collections;

  const collections = collectionsData.map((col, i) => ({
    ...col,
    title: t(T.sections[i].title, lang),
    description: t(T.sections[i].description, lang),
  }));
    ...
    <>
      <section className="pt-32 pb-20 px-6 bg-light">
        <div className="max-w-7xl mx-auto text-center">
          <AnimatedSection>
            <p className="text-[11px] tracking-[0.3em] uppercase text-accent mb-4">{t(T.label, lang)}</p>
            <h1 className="font-[family-name:var(--font-display)] text-4xl sm:text-5xl font-light text-dark mb-6">{t(T.title, lang)}</h1>
            <p className="text-muted text-base max-w-xl mx-auto font-light leading-relaxed">{t(T.subtitle, lang)}</p>
          </AnimatedSection>
        </div>
      </section>
                ...
                <AnimatedSection key={i} delay={i * 0.1}>
                  <div>
                    ...
                    {(item.name || item.price) && (
                      <div className="mt-3">
                        {item.name && <p className="text-dark text-[14px]">{item.name}</p>}
                        {item.price && <p className="text-muted text-[13px] mt-0.5">{item.price}</p>}
                      </div>
                    )}
                  </div>
                </AnimatedSection>
